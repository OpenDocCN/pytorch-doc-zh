...
+   **github**（[*str*](https://docs.python.org/3/library/stdtypes.html#str "(在Python v3.12)")） - 一个格式为<repo_owner/repo_name[:ref]>的字符串，其中包含可选的ref（标签或分支）。如果未指定`ref`，则假定默认分支为`main`（如果存在），否则为`master`。示例：'pytorch/vision:0.10'
