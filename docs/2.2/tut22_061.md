# Pendulum：使用TorchRL编写您的环境和转换

> 原文：[https://pytorch.org/tutorials/advanced/pendulum.html](https://pytorch.org/tutorials/advanced/pendulum.html)

注意

点击[这里](#sphx-glr-download-advanced-pendulum-py)下载完整示例代码

**作者**：[Vincent Moens](https://github.com/vmoens)

创建环境（模拟器或物理控制系统的接口）是强化学习和控制工程的一个整合部分。

TorchRL提供了一套工具在多种情境下实现这一点。本教程演示了如何使用PyTorch和TorchRL从头开始编写一个摆模拟器。它受到了[OpenAI-Gym/Farama-Gymnasium控制库](https://github.com/Farama-Foundation/Gymnasium)中Pendulum-v1实现的启发。

![Pendulum](../Images/6be04eb745758aea3a2462c7d6adf03a.png)

简单摆

关键收获：

+   如何在TorchRL中设计环境：- 编写规格（输入、观察和奖励）；- 实现行为：种子、重置和步骤。

+   转换您的环境输入和输出，并编写您自己的转换；

+   如何使用[`TensorDict`](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "(在tensordict vmain (0.4.0 )版本)")通过`codebase`传递任意数据结构。

    在这个过程中，我们将涉及TorchRL的三个关键组件：
...
为了展示TorchRL环境可以实现的功能，我们将设计一个*无状态*环境。有状态的环境会跟踪最新遇到的物理状态，并依赖于此来模拟状态之间的转换，而无状态的环境期望在每一步提供当前状态，以及采取的动作。TorchRL支持这两种类型的环境，但无状态环境更通用，因此涵盖了TorchRL环境API中更广泛的功能特性。
...
+   我们将首先熟悉环境的属性：其形状（`batch_size`），其方法（主要是[`step()`](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#id4 "(在torchrl vmain (0.4.0 )版本)")、[`reset()`](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#id1 "(在torchrl vmain (0.4.0 )版本)")和[`set_seed()`](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#id3 "(在torchrl vmain (0.4.0 )版本)")）以及最后的规格。

+   在编写完我们的模拟器后，我们将演示如何在训练过程中使用转换。

+   我们将探索从TorchRL的API中产生的新途径，包括：转换输入的可能性，模拟的向量化执行以及通过模拟图进行反向传播的可能性。
...
from tensordict.tensordict import [TensorDict](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict"), [TensorDictBase](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDictBase.html#tensordict.TensorDictBase "tensordict.TensorDictBase")
from torch import nn

from torchrl.data import [BoundedTensorSpec](https://pytorch.org/rl/reference/generated/torchrl.data.BoundedTensorSpec.html#torchrl.data.BoundedTensorSpec "torchrl.data.BoundedTensorSpec"), [CompositeSpec](https://pytorch.org/rl/reference/generated/torchrl.data.CompositeSpec.html#torchrl.data.CompositeSpec "torchrl.data.CompositeSpec"), [UnboundedContinuousTensorSpec](https://pytorch.org/rl/reference/generated/torchrl.data.UnboundedContinuousTensorSpec.html#torchrl.data.UnboundedContinuousTensorSpec "torchrl.data.UnboundedContinuousTensorSpec")
from torchrl.envs import (
    [CatTensors](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.CatTensors.html#torchrl.envs.transforms.CatTensors "torchrl.envs.transforms.transforms.CatTensors"),
    [EnvBase](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#torchrl.envs.EnvBase "torchrl.envs.EnvBase"),
    [Transform](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.Transform.html#torchrl.envs.transforms.Transform "torchrl.envs.transforms.transforms.Transform"),
    ...
    [UnsqueezeTransform](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.UnsqueezeTransform.html#torchrl.envs.transforms.UnsqueezeTransform "torchrl.envs.transforms.transforms.UnsqueezeTransform"),
)
from torchrl.envs.transforms.transforms import _apply_to_composite
from torchrl.envs.utils import [check_env_specs](https://pytorch.org/rl/reference/generated/torchrl.envs.utils.check_env_specs.html#torchrl.envs.utils.check_env_specs "torchrl.envs.utils.check_env_specs"), [step_mdp](https://pytorch.org/rl/reference/generated/torchrl.envs.utils.step_mdp.html#torchrl.envs.utils.step_mdp "torchrl.envs.utils.step_mdp")
...
让我们首先描述手头的问题：我们想要建模一个简单的摆锤，我们可以控制施加在其固定点上的扭矩。我们的目标是将摆锤放在向上位置（按照惯例，角位置为0）并使其保持在该位置静止。为了设计我们的动态系统，我们需要定义两个方程：遵循动作（施加的扭矩）的运动方程和构成我们目标函数的奖励方程。
...
```py
>>> [policy](https://pytorch.org/tensordict/reference/generated/tensordict.nn.TensorDictModule.html#tensordict.nn.TensorDictModule "tensordict.nn.TensorDictModule")([env.reset](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#id1 "torchrl.envs.EnvBase.reset")())
>>> print(tensordict)
TensorDict(
 fields={
 ...
 device=cpu,
 is_shared=False)
>>> [env.step](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#id4 "torchrl.envs.EnvBase.step")(tensordict)
>>> print(tensordict)
TensorDict(
 fields={
 action: Tensor(shape=torch.Size([1]), device=cpu, dtype=torch.float32, is_shared=False),
...
由于我们的目标是将摆锤竖起并保持在那个位置静止，我们的`cost`（负奖励）函数对于接近目标和低速度的位置具有较低的值。实际上，我们希望阻止远离“向上”位置和/或速度远离0的位置。
...
def _step(tensordict):
    ...
    done = [torch.zeros_like](https://pytorch.org/docs/stable/generated/torch.zeros_like.html#torch.zeros_like "torch.zeros_like")(reward, dtype=[torch.bool](https://pytorch.org/docs/stable/tensor_attributes.html#torch.dtype "torch.dtype"))
    out = [TensorDict](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict")(
        {
            "th": new_th,
            "thdot": new_thdot,
...
def _reset(self, tensordict):
    ...
    high_th = [torch.tensor](https://pytorch.org/docs/stable/generated/torch.tensor.html#torch.tensor "torch.tensor")(DEFAULT_X, device=self.device)
    high_thdot = [torch.tensor](https://pytorch.org/docs/stable/generated/torch.tensor.html#torch.tensor "torch.tensor")(DEFAULT_Y, device=self.device)
    low_th = -high_th
    ...
    th = (
        [torch.rand](https://pytorch.org/docs/stable/generated/torch.rand.html#torch.rand "torch.rand")(tensordict.shape, generator=self.rng, device=self.device)
        * (high_th - low_th)
        + low_th
    )
    thdot = (
...
def _make_spec(self, td_params):
    ...
    self.observation_spec = [CompositeSpec](https://pytorch.org/rl/reference/generated/torchrl.data.CompositeSpec.html#torchrl.data.CompositeSpec "torchrl.data.CompositeSpec")(
        th=[BoundedTensorSpec](https://pytorch.org/rl/reference/generated/torchrl.data.BoundedTensorSpec.html#torchrl.data.BoundedTensorSpec "torchrl.data.BoundedTensorSpec")(
            low=-torch.pi,
            high=torch.pi,
            shape=(),
            dtype=[torch.float32](https://pytorch.org/docs/stable/tensor_attributes.html#torch.dtype "torch.dtype"),
        ),
        thdot=[BoundedTensorSpec](https://pytorch.org/rl/reference/generated/torchrl.data.BoundedTensorSpec.html#torchrl.data.BoundedTensorSpec "torchrl.data.BoundedTensorSpec")(
    ...
    self.action_spec = [BoundedTensorSpec](https://pytorch.org/rl/reference/generated/torchrl.data.BoundedTensorSpec.html#torchrl.data.BoundedTensorSpec "torchrl.data.BoundedTensorSpec")(
        low=-td_params["params", "max_torque"],
        high=td_params["params", "max_torque"],
        shape=(1,),
        dtype=[torch.float32](https://pytorch.org/docs/stable/tensor_attributes.html#torch.dtype "torch.dtype"),
    )
    self.reward_spec = [UnboundedContinuousTensorSpec](https://pytorch.org/rl/reference/generated/torchrl.data.UnboundedContinuousTensorSpec.html#torchrl.data.UnboundedContinuousTensorSpec "torchrl.data.UnboundedContinuousTensorSpec")(shape=(*td_params.shape, 1))

def make_composite_from_td([td](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict")):
    # custom function to convert a ``tensordict`` in a similar spec structure
    # of unbounded values.
    composite = [CompositeSpec](https://pytorch.org/rl/reference/generated/torchrl.data.CompositeSpec.html#torchrl.data.CompositeSpec "torchrl.data.CompositeSpec")(
        {
            key: make_composite_from_td(tensor)
            if isinstance(tensor, [TensorDictBase](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDictBase.html#tensordict.TensorDictBase "tensordict.TensorDictBase"))
            else [UnboundedContinuousTensorSpec](https://pytorch.org/rl/reference/generated/torchrl.data.UnboundedContinuousTensorSpec.html#torchrl.data.UnboundedContinuousTensorSpec "torchrl.data.UnboundedContinuousTensorSpec")(
                dtype=tensor.dtype, device=tensor.device, shape=tensor.shape
            )
            for key, tensor in [td.items](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict.items "tensordict.TensorDict.items")()
        },
        shape=[td.shape](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict.shape "tensordict.TensorDict.shape"),
    )
    return composite 
```
...
def _set_seed(self, seed: Optional[int]):
    rng = [torch.manual_seed](https://pytorch.org/docs/stable/generated/torch.manual_seed.html#torch.manual_seed "torch.manual_seed")(seed)
    self.rng = rng 
```

## 将事物包装在一起：[`EnvBase`](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#torchrl.envs.EnvBase "(在torchrl vmain (0.4.0)")类
...
```py
def gen_params(g=10.0, batch_size=None) -> [TensorDictBase](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDictBase.html#tensordict.TensorDictBase "tensordict.TensorDictBase"):
  """Returns a ``tensordict`` containing the physical parameters such as gravitational force and torque or speed limits."""
    if batch_size is None:
        batch_size = []
    [td](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict") = [TensorDict](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict")(
        {
            "params": [TensorDict](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict")(
                {
                    "max_speed": 8,
                    "max_torque": 2.0,
                    "dt": 0.05,
    ...
    if batch_size:
        [td](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict") = [td.expand](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict.expand "tensordict.TensorDict.expand")(batch_size).contiguous()
    return [td](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict") 
```
...
```py
class PendulumEnv([EnvBase](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#torchrl.envs.EnvBase "torchrl.envs.EnvBase")):
    metadata = {
    ...
    def __init__(self, td_params=None, seed=None, device="cpu"):
        ...
        if seed is None:
            seed = [torch.empty](https://pytorch.org/docs/stable/generated/torch.empty.html#torch.empty "torch.empty")((), dtype=[torch.int64](https://pytorch.org/docs/stable/tensor_attributes.html#torch.dtype "torch.dtype")).random_().item()
        self.set_seed(seed)
...
TorchRL提供了一个简单的函数[`check_env_specs()`](https://pytorch.org/rl/reference/generated/torchrl.envs.utils.check_env_specs.html#torchrl.envs.utils.check_env_specs "(在torchrl vmain (0.4.0))")来检查一个（转换后的）环境是否具有与其规格所规定的输入/输出结构相匹配的结构。让我们试一试：

```py
[env](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.TransformedEnv.html#torchrl.envs.transforms.TransformedEnv "torchrl.envs.transforms.transforms.TransformedEnv") = [PendulumEnv](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#torchrl.envs.EnvBase "torchrl.envs.EnvBase")()
...
print("state_spec:", [env.state_spec](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#torchrl.envs.EnvBase.state_spec "torchrl.envs.EnvBase.state_spec"))
...
```py
[td](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict") = [env.reset](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#id1 "torchrl.envs.EnvBase.reset")()
print("reset tensordict", [td](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict")) 
```
...
```py
[td](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict") = [env.rand_step](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#id0 "torchrl.envs.EnvBase.rand_step")([td](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict"))
print("random step tensordict", [td](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict")) 
```
...
为无状态模拟器编写环境转换比为有状态模拟器稍微复杂一些：转换需要在下一次迭代时读取的输出条目需要在下一步调用`meth.step()`之前应用逆转换。这是展示TorchRL转换所有功能的理想场景！
...
TorchRL的转换可能不涵盖所有希望在环境执行后执行的操作。编写一个转换并不需要太多的努力。与环境设计一样，编写转换有两个步骤：

+   正确获取动态（正向和反向）；

+   调整环境规格。

转换可以在两种设置中使用：独立使用时，它可以作为一个[`Module`](https://pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module "(在PyTorch v2.2)")。它也可以附加到一个[`TransformedEnv`](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.TransformedEnv.html#torchrl.envs.transforms.TransformedEnv "(在torchrl vmain (0.4.0))")。类的结构允许在不同上下文中自定义行为。
...
```py
class SinTransform([Transform](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.Transform.html#torchrl.envs.transforms.Transform "torchrl.envs.transforms.transforms.Transform")):
    def _apply_transform(self, obs: [torch.Tensor](https://pytorch.org/docs/stable/tensors.html#torch.Tensor "torch.Tensor")) -> None:
        return obs.sin()

    # The transform must also modify the data at reset time
    def _reset(
        self, tensordict: [TensorDictBase](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDictBase.html#tensordict.TensorDictBase "tensordict.TensorDictBase"), tensordict_reset: [TensorDictBase](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDictBase.html#tensordict.TensorDictBase "tensordict.TensorDictBase")
    ) -> [TensorDictBase](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDictBase.html#tensordict.TensorDictBase "tensordict.TensorDictBase"):
    ...
    def transform_observation_spec(self, observation_spec):
        return [BoundedTensorSpec](https://pytorch.org/rl/reference/generated/torchrl.data.BoundedTensorSpec.html#torchrl.data.BoundedTensorSpec "torchrl.data.BoundedTensorSpec")(
            low=-1,
            high=1,
...
class CosTransform([Transform](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.Transform.html#torchrl.envs.transforms.Transform "torchrl.envs.transforms.transforms.Transform")):
    def _apply_transform(self, obs: [torch.Tensor](https://pytorch.org/docs/stable/tensors.html#torch.Tensor "torch.Tensor")) -> None:
        return obs.cos()
...
t_sin = [SinTransform](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.Transform.html#torchrl.envs.transforms.Transform "torchrl.envs.transforms.transforms.Transform")(in_keys=["th"], out_keys=["sin"])
t_cos = [CosTransform](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.Transform.html#torchrl.envs.transforms.Transform "torchrl.envs.transforms.transforms.Transform")(in_keys=["th"], out_keys=["cos"])
[env](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.TransformedEnv.html#torchrl.envs.transforms.TransformedEnv "torchrl.envs.transforms.transforms.TransformedEnv").append_transform(t_sin)
[env](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.TransformedEnv.html#torchrl.envs.transforms.TransformedEnv "torchrl.envs.transforms.transforms.TransformedEnv").append_transform(t_cos) 
```

将观察结果连接到“observation”条目上。`del_keys=False`确保我们保留这些值供下一次迭代使用。

```py
[cat_transform](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.CatTensors.html#torchrl.envs.transforms.CatTensors "torchrl.envs.transforms.transforms.CatTensors") = [CatTensors](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.CatTensors.html#torchrl.envs.transforms.CatTensors "torchrl.envs.transforms.transforms.CatTensors")(
    in_keys=["sin", "cos", "thdot"], dim=-1, out_key="observation", del_keys=False
)
[env](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.TransformedEnv.html#torchrl.envs.transforms.TransformedEnv "torchrl.envs.transforms.transforms.TransformedEnv").append_transform([cat_transform](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.CatTensors.html#torchrl.envs.transforms.CatTensors "torchrl.envs.transforms.transforms.CatTensors")) 
```
...
def simple_rollout(steps=100):
    # preallocate:
    data = [TensorDict](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict")({}, [steps])
    # reset
    _data = [env.reset](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#id1 "torchrl.envs.EnvBase.reset")()
    for i in range(steps):
        _data["action"] = [env.action_spec.rand](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#torchrl.envs.EnvBase.action_spec "torchrl.envs.EnvBase.action_spec")()
        _data = [env.step](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#id4 "torchrl.envs.EnvBase.step")(_data)
        data[i] = _data
        _data = [step_mdp](https://pytorch.org/rl/reference/generated/torchrl.envs.utils.step_mdp.html#torchrl.envs.utils.step_mdp "torchrl.envs.utils.step_mdp")(_data, keep_other=True)
    return data
...
我们教程的最后一个未探索的部分是我们在TorchRL中批量计算的能力。因为我们的环境对输入数据形状没有任何假设，所以我们可以无缝地在数据批次上执行它。更好的是：对于像我们的摆锤这样的非批量锁定环境，我们可以在不重新创建环境的情况下即时更改批量大小。为此，我们只需生成所需形状的参数。

```py
batch_size = 10  # number of environments to be executed in batch
[td](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict") = [env.reset](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#id1 "torchrl.envs.EnvBase.reset")([env](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.TransformedEnv.html#torchrl.envs.transforms.TransformedEnv "torchrl.envs.transforms.transforms.TransformedEnv").gen_params(batch_size=[batch_size]))
print("reset (batch size of 10)", [td](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict"))
[td](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict") = [env.rand_step](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#id0 "torchrl.envs.EnvBase.rand_step")([td](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict"))
print("rand step (batch size of 10)", [td](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict")) 
```
...
```py
[rollout](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict") = [env.rollout](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#id2 "torchrl.envs.EnvBase.rollout")(
    3,
    auto_reset=False,  # we're executing the reset out of the ``rollout`` call
    tensordict=[env.reset](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#id1 "torchrl.envs.EnvBase.reset")([env](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.TransformedEnv.html#torchrl.envs.transforms.TransformedEnv "torchrl.envs.transforms.transforms.TransformedEnv").gen_params(batch_size=[batch_size])),
)
print("rollout of len 3 (batch size of 10):", [rollout](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict")) 
```
...
然而，这只是一个非常简单的例子，展示了如何在TorchRL中使用自定义环境编写训练循环。

让我们首先编写策略网络：

```py
[torch.manual_seed](https://pytorch.org/docs/stable/generated/torch.manual_seed.html#torch.manual_seed "torch.manual_seed")(0)
[env.set_seed](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.TransformedEnv.html#torchrl.envs.transforms.TransformedEnv.set_seed "torchrl.envs.transforms.TransformedEnv.set_seed")(0)

[net](https://pytorch.org/docs/stable/generated/torch.nn.Sequential.html#torch.nn.Sequential "torch.nn.Sequential") = [nn.Sequential](https://pytorch.org/docs/stable/generated/torch.nn.Sequential.html#torch.nn.Sequential "torch.nn.Sequential")(
    [nn.LazyLinear](https://pytorch.org/docs/stable/generated/torch.nn.LazyLinear.html#torch.nn.LazyLinear "torch.nn.LazyLinear")(64),
    [nn.Tanh](https://pytorch.org/docs/stable/generated/torch.nn.Tanh.html#torch.nn.Tanh "torch.nn.Tanh")(),
    [nn.LazyLinear](https://pytorch.org/docs/stable/generated/torch.nn.LazyLinear.html#torch.nn.LazyLinear "torch.nn.LazyLinear")(64),
    [nn.Tanh](https://pytorch.org/docs/stable/generated/torch.nn.Tanh.html#torch.nn.Tanh "torch.nn.Tanh")(),
    [nn.LazyLinear](https://pytorch.org/docs/stable/generated/torch.nn.LazyLinear.html#torch.nn.LazyLinear "torch.nn.LazyLinear")(64),
    [nn.Tanh](https://pytorch.org/docs/stable/generated/torch.nn.Tanh.html#torch.nn.Tanh "torch.nn.Tanh")(),
    [nn.LazyLinear](https://pytorch.org/docs/stable/generated/torch.nn.LazyLinear.html#torch.nn.LazyLinear "torch.nn.LazyLinear")(1),
)
[policy](https://pytorch.org/tensordict/reference/generated/tensordict.nn.TensorDictModule.html#tensordict.nn.TensorDictModule "tensordict.nn.TensorDictModule") = [TensorDictModule](https://pytorch.org/tensordict/reference/generated/tensordict.nn.TensorDictModule.html#tensordict.nn.TensorDictModule "tensordict.nn.TensorDictModule")(
    [net](https://pytorch.org/docs/stable/generated/torch.nn.Sequential.html#torch.nn.Sequential "torch.nn.Sequential"),
    in_keys=["observation"],
    out_keys=["action"],
) 
```
...
```py
[optim](https://pytorch.org/docs/stable/generated/torch.optim.Adam.html#torch.optim.Adam "torch.optim.Adam") = [torch.optim.Adam](https://pytorch.org/docs/stable/generated/torch.optim.Adam.html#torch.optim.Adam "torch.optim.Adam")([policy.parameters](https://pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module.parameters "torch.nn.Module.parameters")(), lr=2e-3) 
```
...
在训练循环结束时，我们应该有一个接近0的最终奖励，这表明摆锤向上并保持静止。

```py
batch_size = 32
pbar = tqdm.tqdm(range(20_000 // batch_size))
[scheduler](https://pytorch.org/docs/stable/generated/torch.optim.lr_scheduler.CosineAnnealingLR.html#torch.optim.lr_scheduler.CosineAnnealingLR "torch.optim.lr_scheduler.CosineAnnealingLR") = [torch.optim.lr_scheduler.CosineAnnealingLR](https://pytorch.org/docs/stable/generated/torch.optim.lr_scheduler.CosineAnnealingLR.html#torch.optim.lr_scheduler.CosineAnnealingLR "torch.optim.lr_scheduler.CosineAnnealingLR")([optim](https://pytorch.org/docs/stable/generated/torch.optim.Adam.html#torch.optim.Adam "torch.optim.Adam"), 20_000)
logs = defaultdict(list)

for _ in pbar:
    [init_td](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict") = [env.reset](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#id1 "torchrl.envs.EnvBase.reset")([env](https://pytorch.org/rl/reference/generated/torchrl.envs.transforms.TransformedEnv.html#torchrl.envs.transforms.TransformedEnv "torchrl.envs.transforms.transforms.TransformedEnv").gen_params(batch_size=[batch_size]))
    [rollout](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict") = [env.rollout](https://pytorch.org/rl/reference/generated/torchrl.envs.EnvBase.html#id2 "torchrl.envs.EnvBase.rollout")(100, [policy](https://pytorch.org/tensordict/reference/generated/tensordict.nn.TensorDictModule.html#tensordict.nn.TensorDictModule "tensordict.nn.TensorDictModule"), tensordict=[init_td](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict"), auto_reset=False)
    [traj_return](https://pytorch.org/docs/stable/tensors.html#torch.Tensor "torch.Tensor") = [rollout](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict")["next", "reward"].mean()
    (-[traj_return](https://pytorch.org/docs/stable/tensors.html#torch.Tensor "torch.Tensor")).backward()
    [gn](https://pytorch.org/docs/stable/tensors.html#torch.Tensor "torch.Tensor") = [torch.nn.utils.clip_grad_norm_](https://pytorch.org/docs/stable/generated/torch.nn.utils.clip_grad_norm_.html#torch.nn.utils.clip_grad_norm_ "torch.nn.utils.clip_grad_norm_")([net.parameters](https://pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module.parameters "torch.nn.Module.parameters")(), 1.0)
    ...
    pbar.set_description(
        f"reward: {[traj_return](https://pytorch.org/docs/stable/tensors.html#torch.Tensor "torch.Tensor"): 4.4f}, "
        f"last reward: {[rollout](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict")[...,  -1]['next',  'reward'].mean(): 4.4f}, gradient norm: {[gn](https://pytorch.org/docs/stable/tensors.html#torch.Tensor "torch.Tensor"): 4.4}"
    )
    logs["return"].append([traj_return](https://pytorch.org/docs/stable/tensors.html#torch.Tensor "torch.Tensor").item())
    logs["last_reward"].append([rollout](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "tensordict.TensorDict")[..., -1]["next", "reward"].mean().item())
...
![returns, last reward](../Images/9f96ad35f80739a0aa9e5611f81b2cd0.png)
...
+   编码环境时需要注意的四个基本组件（`step`、`reset`、种子和构建规范）。我们看到这些方法和类如何与[`TensorDict`](https://pytorch.org/tensordict/reference/generated/tensordict.TensorDict.html#tensordict.TensorDict "(在tensordict vmain (0.4.0))")类交互；

+   如何测试环境是否正确编码使用[`check_env_specs()`](https://pytorch.org/rl/reference/generated/torchrl.envs.utils.check_env_specs.html#torchrl.envs.utils.check_env_specs "(在torchrl vmain (0.4.0))");
...
[`下载Python源代码：pendulum.py`](../_downloads/0c4dc681209d8c964aae9de5e477d280/pendulum.py)

[`下载Jupyter笔记本：pendulum.ipynb`](../_downloads/8016e5cfa285bd92b9684c45552fffcc/pendulum.ipynb)
